...
security_group_id=$(aws ec2 describe-security-groups --group-names "comunidad-dev" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)
if [ $? -eq 0 ]; then
  echo "[OK] Security Group comunidad-dev foi criado"
  
  # Validar inbound rule para o security group 'comunidad-dev'
  ...
  # Validar outbound rule para o security group 'comunidad-dev'
  ...
  echo ">[ERRO] Não achei o security group comunidad-dev. Ele foi criado?"
fi
